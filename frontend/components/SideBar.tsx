...
const SidebarItem = ({ chatObject }) => (
  <div className="group">
    <Link href={`/c/${chatObject.id}`} className="flex items-center rounded-lg py-2 relative text-sm max-w-full whitespace-nowrap text-ellipsis">
        {chatObject.label}
    </Link>
  </div>
);

const SidebarSection = ({ title, items, session, setHistory, setAccount }) => {  
